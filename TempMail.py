...
class TempMail:
    ...
    def read_emails(self):
        if self.__driver.current_url == 'https://temp-mail.org/en/' and len(self.__emails_xpath) > 0:
            for email_xpath in self.__emails_xpath:
                ...
                cur_email_body = self.__driver.find_element_by_xpath(
                    '//*[@id="tm-body"]/main/div[1]/div/div[2]/div[2]/div/div[1]/div/div[2]/div[3]/div').text
                self.__content[cur_email_subject] = cur_email_body
                self.__driver.get('https://temp-mail.org/en/')
                time.sleep(3)
        else:
            self.__driver.get('https://temp-mail.org/en/')
